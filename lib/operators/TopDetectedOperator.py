...
class TopDetectedOperator:
    """
    General-purpose operator for calculating top X items with grouping and aggregation.

    This operator performs the following steps:
    1. Detect and flag duplicate records based on specified deduplication columns
       - This is a dedicated data quality check
       - Duplicates are flagged with is_unique=False (first occurrence gets is_unique=True)
       - All original data is preserved, no deletion occurs
       - Duplicate findings are logged
    2. Group by specified columns (similar to SQL: SELECT group_cols, COUNT(count_col) GROUP BY group_cols WHERE is_unique=True)
    ...
    Config structure:
        processing:
            ...
            groupby_columns: List of column names to group by for aggregation
            dedup_columns: List of column names to check for duplicates; by default, same as groupby_columns
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.groupby_columns = self.processing_config.get('groupby_columns', [])
        self.dedup_columns = self.processing_config.get('dedup_columns', 'groupby_columns')
    ...
    def calculate(self, data_rdd: RDD) -> RDD:
        """
        Main calculation method: read -> detect & flag -> aggregate -> rank & select top.
    ...
    def _detect_and_flag(self, rdd: RDD) -> RDD:
        ...
        3. Adds an 'is_unique' flag to each row (True for unique/first occurrence, False for duplicates)
        ...
        def flag_duplicates(key_group):
            ...
            if len(rows_list) > 1:
                logger.warning(
                    f"Duplicate detected: {len(rows_list)} records found for key {key}. "
                    f"Flagging first occurrence as unique, marking {len(rows_list) - 1} as duplicate(s)."
                )

            # Return all rows with flags: first one is unique (True), rest are duplicates (False)
            flagged_rows = []
    ...
    def _calculate_top_items(self, data_rdd: RDD) -> RDD:
        ...
        Args:
            data_rdd: RDD of tuples (row, is_unique_flag) where is_unique_flag is boolean
        ...
        def create_group_key(row):
            group_vals = tuple(getattr(row, col) for col in self.groupby_columns)
            rank_val = getattr(row, self.rank_column)
            return (group_vals, rank_val)

        # Group and count occurrences - reduceByKey sums up counts for same (group, item) combinations
    ...
    def _rank_and_select_top(self, group_data):
        ...
        Args:
            group_data: Tuple of (group_values, items_iterator) where items are (item_value, count)

        Returns:
            List of tuples: (*group_values, rank, item_value) for top X items only
        """
        group_vals, items = group_data

        # Sort by count (descending) then by item value (ascending) for tie-breaking
        sorted_items = sorted(items, key=lambda x: (-x[1], x[0]))

        # Take top X items and assign ranks (1-based ranking)
        ranked_items = []
        for rank, (item_value, count) in enumerate(sorted_items[:self.top_x], 1):
